...
def scrape_tavex_gold_products() -> list:
    """
    ...
    url = config.TAVEX_BASE_URL + "/zlato/"
    ...
    logger.info("Found %d potential products", len(options))
    ...
    for option in options:
        ...
        try:
            ...
            logger.warning("Error parsing data for %s: %s", name, e)
            error_count += 1
            continue

    logger.info("Successfully scraped %d products, %d errors", len(gold_products), error_count)
    return gold_products


def save_to_json(gold_products: list, output_file: str = "tavex_gold_products.json") -> bool:
    """
    ...
    try:
        ...
        logger.exception("Error saving data to %s: %s", output_file, e)
        return False
...
def main():
    ...
    parser.add_argument(
        "--output",
        "-o",
        default="tavex_gold_products.json",
    ...
    avg_spread = (
        sum(product["spread_percentage"] for product in gold_products)
        / len(gold_products)
    )
